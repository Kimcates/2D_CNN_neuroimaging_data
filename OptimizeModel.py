...
pickle_in.close()
pickle_in = open ("gm_imgs_test", "rb")
gm_imgs_train = pickle.load(pickle_in)
pickle_in.close()
...
for dense_layer in dense_layers:
    for layer_size in layer_sizes:
        for conv_layer in conv_layers:
            name = "stepsepoch6000-dropout-.01L2-regularizers-{}-conv-{}-nodes-{}-dense{}".format(conv_layer, layer_size, dense_layer, int(time.time()))
            ...
            classifier.add(Conv2D(layer_size, (3, 3), input_shape = (64, 64, 3), kernel_regularizer=l2(0.01), bias_regularizer=l2(0.01), activation = 'relu'))
            # Pooling
            classifier.add(MaxPooling2D(pool_size = (2, 2)))
            classifier.add(Dropout(0.25))

            for layer in range(conv_layer - 1):
                classifier.add(Conv2D(layer_size,3,3, kernel_regularizer=l2(0.01), bias_regularizer=l2(0.01), activation='relu'))
                classifier.add(MaxPooling2D(pool_size=(2,2)))
                classifier.add(Dropout(0.25))
